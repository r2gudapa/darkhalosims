...
// This script analyses the full snapshot, but skips every N particles (the skip size).
// Set the skip size below.

#define skip_size 4 // MAKE THIS CAPITALIZED
...
#define FIX_HEADER 0 
int skip_rows[skip_size*3*sizeof(float)];
...
#define SKIP fread(&dummy, sizeof(dummy), 1, fd);
  ...
  for(i = 0, pc = 1; i < files; i++, pc = pc_new)
	...
	if (FIX_HEADER) {
		...
		header1.npart[1] = header1.npart[5];
		header1.npart[5] = 0;
		header1.mass[1] = header1.mass[3];
		header1.mass[3] = 0.0;
		header1.npartTotal[1] = header1.npartTotal[5];
	  ...
	  for(n = 0; n < header1.npart[k];)
	    {
		    if (n % skip_size == 0) {
			    ...
			    fread(skip_rows, sizeof(float) * 3 * (skip_size - 1), 1, fd);
			    n = n + (skip_size - 1);
		    } else {
			    int rows_left = header1.npart[k] - n;
			    fread(skip_rows, sizeof(float) * 3 * rows_left, 1, fd);
			    n++;
		    }
	    }
	}
	  ...
	  for(n = 0; n < header1.npart[k];)
	    {
	      	if (n % skip_size == 0) { 
			fread(&Id[pc_new], sizeof(int), 1, fd);
			if (Id[pc_new] == 0) { Id[pc_new] = 1; }
			pc_new++;
			n++; 
	      	} else if ((header1.npart[k] - n) >= skip_size) {
			fread(skip_rows, sizeof(int) * 1 * (skip_size - 1), 1, fd);
			n = n + (skip_size - 1);
		} else {
			int rows_left = header1.npart[k] - n;
			fread(skip_rows, sizeof(int) * 1 * rows_left, 1, fd);
			n++;
		}
	    }
	}
